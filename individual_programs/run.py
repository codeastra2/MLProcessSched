...
def run_set(path):
	...
	for file in os.listdir(path):
		...
		output_1 = create_csv_row(file)

		''' 3 '''
		run += 'wait'
		output_2 = Popen(run, stdout=PIPE, shell = True, cwd = path).communicate()[0].strip()

		''' 4 '''
		output_2 = output_2.split(',')[1:]
		output_2 = [int(x) for x in output_2]
		...
		with open(OUTPUT + date_time + '.csv', 'a') as f:
			...
			if len(output_1 + output_2) != 34:
				raise Exception('Less attributes')
			else:
				w.writerow(output_1 + output_2)
	...
	for directory in directories:
		...
		try:
			...
			with open(ERROR + 'error.txt', 'a') as f:
				f.write(date_time + '\t' + 'Running New\t' + str(directory) + '\t' + str(e) + '\n')
